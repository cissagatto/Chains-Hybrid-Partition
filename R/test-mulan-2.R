...
#   Parameters                                                               #
##############################################################################
build.mulan <- function(parameters){
  ...
  while(f<=parameters$Number.Folds){
    ...
    while(g<=best.part.info.f$num.group){
      ...
      if(g==1){
        ...
        arg.targets = paste(info.clusters$label.start, "-", 
                            info.clusters$label.end, sep="")
        ...
        if(all.total.labels.g$totalLabels==1){
          ...
          train.cluster.mldr <- mldr::mldr_from_dataframe(dataframe = train.dataset, 
                                                          labelIndices = seq(info.clusters$label.start, 
                                                                             info.clusters$label.end, by=1), 
                                                          name = str.train)
...
#   Parameters                                                               #
##############################################################################
gather.predicts.mulan <- function(parameters){
